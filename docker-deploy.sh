...
echo "🔨 Building echo-relay-java for user: $USER_NAME, tag: $TAG_NAME"
echo
docker build ./ -t $USER_NAME/echo-relay-java:$TAG_NAME

echo "🔐 Dockerhub login for user: $USER_NAME"
docker login -u "$USER_NAME" docker.io
docker push $USER_NAME/echo-relay-java:$TAG_NAME
